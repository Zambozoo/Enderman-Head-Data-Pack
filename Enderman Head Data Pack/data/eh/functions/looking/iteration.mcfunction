...
execute if score $filter find_look.temp matches 1.. as @e[tag=find_looking.candidate,tag=find_looking.result] run function eh:looking/check_filter
...
execute store success score $success find_look.temp if predicate eh:looking_at/filter
...
scoreboard players operation $filter find_look.temp *= $2 find_look.temp
execute if score $filter find_look.temp matches 0 run scoreboard players set $filter find_look.temp 1

execute if entity @e[tag=find_looking.candidate,tag=find_looking.result,limit=1] if score $filter find_look.temp <= $max find_look.temp run function eh:looking/iteration
